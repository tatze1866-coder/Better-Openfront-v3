...
import { Game, TURN_MS, SPAWN_TURNS, CITY_COST, FORT_COST, MAX_BOATS, BOT_LEVELS } from './engine.js';
...
wireSeg('lobbyLevelSeg', l => {
  lobbyLevel = l;
  if (isHost && ws) wsSend({ t: 'bots', n: +$('lobbyBots').value, level: l });
});
...
$('lobbyBots').addEventListener('input', e => {
  ...
  if (isHost && ws) wsSend({ t: 'bots', n: +e.target.value, level: lobbyLevel });
});
...
$('btnSolo').addEventListener('click', () => {
  ...
  startGame(seed, players, 0, false);
});
...
function handleServerMsg(m) {
  switch (m.t) {
    ...
    case 'error':
      ...
      break;
    case 'start':
      startGame(m.seed, m.players, findMyIdx(), true);
      break;
    case 'turn':
      turnQueue.push(m);
      processTurnQueue();
...
// ---------- Spiel ----------
function startGame(seed, players, idx, isOnline) {
  myIdx = idx;
  online = isOnline;
  turnQueue = [];
  localPending = [];
...
function updateBuildButtons() {
  $('btnCity').classList.toggle('build-active', buildMode === 'city');
  $('btnFort').classList.toggle('build-active', buildMode === 'fort');
  $('canvas').style.cursor = buildMode ? 'copy' : 'crosshair';
}
$('btnCity').textContent = `🏙 Stadt (${CITY_COST})`;
$('btnFort').textContent = `🛡 Festung (${FORT_COST})`;
$('btnCity').addEventListener('click', () => setBuildMode('city'));
$('btnFort').addEventListener('click', () => setBuildMode('fort'));
...
window.addEventListener('keydown', e => {
  ...
  if (k === '1') { setBuildMode('city'); showToast(buildMode ? 'Stadt: Zielfeld anklicken.' : 'Baumodus beendet.'); }
  else if (k === '2') { setBuildMode('fort'); showToast(buildMode ? 'Festung: Zielfeld anklicken.' : 'Baumodus beendet.'); }
});
...
function buildCtxItems(owner, cell) {
  ...
  if (cell >= 0 && owner === myIdx) {
    for (const kind of ['city', 'fort']) {
      const err = game.canBuildAt(myIdx, cell, kind);
      const label = kind === 'city' ? `🏙 Stadt bauen (${CITY_COST})` : `🛡 Festung bauen (${FORT_COST})`;
      items.push({
        label, disabled: !!err, hint: err || '',
        action: () => sendIntent({ type: 'build', kind, cell })
      });
    }
  }
